...
[X_H, errList] = FaLRTC(Xm.data,logical(M.data), alpha, 0.5, 1, 1, maxIter, epsilon);
T(k)=toc;
Xr{k}=tensor(X_H);
%% TenALS 2014
disp('TenALS...')
...
tic
r_init=2;% 1 2 5 10
[V{1} V{2} V{3} s dist] = TenALS(Xm.data, M.data, r_init, 1, 20, 1e-5);% 100 iter
T(k)=toc;
...
opts.rank_adj = 1*ones(1,3);   opts.rank_min = 10*ones(1,3);    opts.rank_max = [20 10 5];%[30 20 8]
...
Par2.rho = 1.01;    Par2.mu = 1e-5; Par2.lambda = 1; % 0.01 is even better
Par2.rank=size(X);
...
k=k+1; 
s_G=[50 50 7];
s_NN{1}=[25 s_G(1) size(Xm,1)];
s_NN{2}=[25 s_G(2) size(Xm,2)];
s_NN{3}=[6 s_G(3) size(Xm,3)];
lambda_w=[1 1 1]*0.001;
lambda_z=[1 1 1]*0.001;
lambda_c=[0.001];
