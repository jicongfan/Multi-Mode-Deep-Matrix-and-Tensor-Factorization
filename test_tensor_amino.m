...
% % opts.rank_adj = 1*ones(1,3);   opts.rank_min = 10*ones(1,3);    opts.rank_max = [5 20 10];
...
% % Par2.rho = 1.01;    Par2.mu = 1e-5; Par2.lambda = 0.01; % 0.01 is even better
% % Par2.rank=size(X);
...
k=k+1; 
s_G=[5 50 15];
s_NN{1}=[4 s_G(1) size(Xm,1)];
s_NN{2}=[25 s_G(2) size(Xm,2)];%10
s_NN{3}=[10 s_G(3) size(Xm,3)];% 5
